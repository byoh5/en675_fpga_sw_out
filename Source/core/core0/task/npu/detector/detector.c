...
void build_box_info(box_t* box, tensor_t* loc, tensor_t* prior, int idx)
{
    ...
    int x, y, x_center, y_center, x_offset, y_offset;
    ...
    _log2_scale = prior->log2_scale;
    if (_log2_scale >= 0)
    {
        x_center = _prior[0] << _log2_scale;
        y_center = _prior[1] << _log2_scale;
    }
    else
    {
        x_center = _prior[0] >> (-_log2_scale);
...
//	printf("numofobject %d\n",*num);
	...
	for (int i = 0 ; i < r1->n ; i++)
	{
		...
		*(data+(i*6)) = (uint32_t)b->x_min;
		*(data+(i*6)+1) = (uint32_t)b->x_max;
		*(data+(i*6)+2) = (uint32_t)b->y_min;
		*(data+(i*6)+3) = (uint32_t)b->y_max;
		*(data+(i*6)+4) = (uint32_t)r1->score[i];
		*(data+(i*6)+5) = (uint32_t)r1->class[i];
...
int main(int argc, char** argv)
{
    ...
    void* loc_buf_32 = (void*)malloc(sizeof(uint8_t)*1*NUM_BOX*32);
    void* score_buf_32 = (void*)malloc(sizeof(uint8_t)*1*NUM_BOX*32);

    void* score_buf_ref = (void*)malloc(sizeof(uint8_t)*1*NUM_BOX*4);
    void* loc_buf_ref = (void*)malloc(sizeof(uint8_t)*1*NUM_BOX*4);

    int log2_prior_scl, log2_score_scl, log2_loc_scl; 
    int num_class;

    char target_path[256];
    char target_score_path[256];
    char target_loc_path[256];

    if (argc == 1) 
        sprintf(target_path, "./sample/zidane");
    else if (argc == 2)
        sprintf(target_path, "%s", argv[1]);
    else {
        printf("usage:\n");
        printf("     test [test_oact_path];\n");
        printf("Default test_oact_path: ./sample/zidane\n");
        return 1;
    }

    //Target score, loc
    sprintf(target_score_path, "%s/%s", target_path, "Concat_290.oa.bin");
    in = fopen(target_score_path, "rb");
    if (in == NULL) {
        enlight_log("cannot open %s\n", target_score_path);
        return 1;
    }
    fread(score_buf_32, 1, 1*NUM_BOX*32, in);
    fclose(in);

    sprintf(target_loc_path, "%s/%s", target_path, "Concat_291.oa.bin");
    in = fopen(target_loc_path, "rb");
    if (in == NULL) {
        enlight_log("cannot open %s\n", target_loc_path);
        return 1;
    }
    fread(loc_buf_32, 1, 1*NUM_BOX*32, in);
    fclose(in);
    ...
    log2_prior_scl = 1;
    log2_score_scl = 5;
    log2_loc_scl = 4 - 3; //multiplied by 8 at matmul_var
    num_class = 21;
    ...
    int num_class) 
{
    ...
    // [6, H, W, 32] -> [H, W, 6*21]
    arrange_tensor(score_buf_32, score_buf, num_class);
    // [6, H, W, 4] -> [H, W, 6*4]
    arrange_tensor(loc_buf_32, loc_buf, 4);
    // loc * [0.1, 0.1, 0.2, 0.2] * (4)
    matmul_var(loc_buf);
    log2_loc_scl = log2_loc_scl - 3; //multiplied by 8 at matmul_var

    //enlight_log("p:%d, l:%d s:%d\n", log2_prior_scl , log2_loc_scl , log2_score_scl);
