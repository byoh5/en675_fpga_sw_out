                       ...
                       t_u8 wait_option)
{
    ...
    ewl_os_memcpy(sys_cfg, &info->param.bss_config, sizeof(mlan_uap_bss_param));
...
#define MIN_AP_CFG_CMD_LEN   16 /* strlen("ASCII_CMD=AP_CFG") */
    ...
    //enx_os_memset(buf, 0, MAX_BUF_LEN);
    //enx_os_memcpy(buf, wrq->u.data.pointer, wrq->u.data.length);
 ...
 */
int
woal_uap_do_priv_ioctl(moal_private *priv, struct ifreq *req, int cmd)
{
