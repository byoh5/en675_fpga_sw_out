 ...
 */
static int
woal_uap_addba_param(moal_private *priv, struct ifreq *req)
{
    ...
    ENTER();
    ewl_os_memset(&param, 0, sizeof(param));
    ...
    ewl_os_memcpy(&param, req->ifr_data, sizeof(param));
    ...
    /* Copy to user */
    ewl_os_memcpy(req->ifr_data, &param, sizeof(param));
 ...
 */
static int
woal_uap_aggr_priotbl(moal_private *priv, struct ifreq *req)
{
    mlan_ioctl_req *ioctl_req = NULL;
    mlan_ds_11n_cfg *cfg_11n = NULL;
    aggr_prio_tbl param;
 ...
 */
static int
woal_uap_addba_reject(moal_private *priv, struct ifreq *req)
{
    mlan_ioctl_req *ioctl_req = NULL;
    mlan_ds_11n_cfg *cfg_11n = NULL;
 ...
 */
static int
woal_uap_deep_sleep(moal_private *priv, struct ifreq *req)
{
    mlan_ioctl_req *ioctl_req = NULL;
 ...
 */
static int
woal_uap_ioctl(moal_private *priv, struct ifreq *req)
{
    int ret = 0;
    t_u32 subcmd = 0;
    ENTER();
    ...
    ewl_os_memcpy(&subcmd, req->ifr_data, sizeof(subcmd));

    PRINTM(MIOCTL, "ioctl subcmd=%d\n", (int) subcmd);
    switch (subcmd) {
    case UAP_ADDBA_PARA:
        ret = woal_uap_addba_param(priv, req);
        break;
    case UAP_AGGR_PRIOTBL:
        ret = woal_uap_aggr_priotbl(priv, req);
        break;
    case UAP_ADDBA_REJECT:
        ret = woal_uap_addba_reject(priv, req);
        break;
    case UAP_DEEP_SLEEP:
        ret = woal_uap_deep_sleep(priv, req);
        break;
    default:
        break;
    }
 ...
 */
static int
woal_uap_sta_deauth_ioctl(moal_private *priv, struct ifreq *req)
{
    mlan_ioctl_req *ioctl_req = NULL;
    ...
    ewl_os_memset(&deauth_param, 0, sizeof(mlan_deauth_param));
    /* Sanity check */
    if (req->ifr_data == NULL) {
    ...
    ewl_os_memcpy(&deauth_param, req->ifr_data, sizeof(mlan_deauth_param));
    ...
    ewl_os_memcpy(&bss->param.deauth_param, &deauth_param, sizeof(mlan_deauth_param));
    if (MLAN_STATUS_SUCCESS !=
        woal_request_ioctl(priv, ioctl_req, MOAL_IOCTL_WAIT)) {
        ret = -EFAULT;
        ewl_os_memcpy(req->ifr_data, &ioctl_req->status_code, sizeof(t_u32));
    }
 ...
 */
static int
woal_uap_bss_ctrl_ioctl(moal_private * priv, struct ifreq *req)
{
    int ret = 0, data = 0;
    ...
    ewl_os_memcpy(&data, req->ifr_data, sizeof(data));
 ...
 */
static int
woal_uap_power_mode_ioctl(moal_private * priv, struct ifreq *req)
{
    mlan_ioctl_req *ioctl_req = NULL;
    ...
    ewl_os_memset(&ps_mgmt, 0, sizeof(mlan_ds_ps_mgmt));
    ...
    ewl_os_memcpy(&ps_mgmt, req->ifr_data, sizeof(mlan_ds_ps_mgmt));
    ...
    if (ps_mgmt.flags) {
        ioctl_req->action = MLAN_ACT_SET;
        ewl_os_memcpy(&pm_cfg->param.ps_mgmt, &ps_mgmt, sizeof(mlan_ds_ps_mgmt));
    } else {
        ioctl_req->action = MLAN_ACT_GET;
    }
 ...
 */
static int
woal_uap_bss_cfg_ioctl(moal_private * priv, struct ifreq *req)
{
    int ret = 0;
    mlan_ds_bss *bss = NULL;
    mlan_ioctl_req *ioctl_req = NULL;
    ...
    /* Get action */
    ewl_os_memcpy(&action, req->ifr_data + offset, sizeof(action));
    ...
    if (ioctl_req->action == MLAN_ACT_SET)
        ...
        ewl_os_memcpy(&bss->param.bss_config, req->ifr_data + offset,sizeof(mlan_uap_bss_param)); 
    ...
    if (ioctl_req->action == MLAN_ACT_GET) {
        ...
        ewl_os_memcpy(req->ifr_data + offset, &bss->param.bss_config,sizeof(mlan_uap_bss_param)); 
    }
done:
    if (ioctl_req)
        ewl_os_free(ioctl_req);
 ...
 */
static int
woal_uap_get_sta_list_ioctl(moal_private * priv, struct ifreq *req)
{
    int ret = 0;
...
woal_uap_set_wapi_key_ioctl(moal_private * priv, wapi_msg * msg)
{
    ...
    ewl_os_memcpy(sec->param.encrypt_key.mac_addr, key_msg->mac_addr, ETH_ALEN);
    sec->param.encrypt_key.key_index = key_msg->key_id;
    ewl_os_memcpy(sec->param.encrypt_key.key_material, key_msg->key,
...
woal_uap_set_wapi_flag_ioctl(moal_private * priv, wapi_msg * msg)
{
    ...
    if (msg->msg[0] & WAPI_MODE_PSK) {
        ewl_os_memcpy(misc->param.gen_ie.ie_data, wapi_psk_ie, misc->param.gen_ie.len);
    } else if (msg->msg[0] & WAPI_MODE_CERT) {
        ewl_os_memcpy(misc->param.gen_ie.ie_data, wapi_cert_ie,
 ...
 */
static int
woal_uap_set_wapi(moal_private *priv, struct ifreq *req)
{
    wapi_msg msg;
    int ret = 0;
    ...
    ewl_os_memset(&msg, 0, sizeof(msg));

    ewl_os_memcpy(&msg, req->ifr_data, sizeof(msg));
 ...
 */
static int
woal_uap_hs_cfg(moal_private *priv, struct ifreq *req)
{
    ...
    ewl_os_memset(&hscfg, 0, sizeof(mlan_ds_hs_cfg));
    ewl_os_memset(&hs_cfg, 0, sizeof(ds_hs_cfg));
    ...
    ewl_os_memcpy(&hs_cfg, req->ifr_data, sizeof(ds_hs_cfg));
    ...
    if (!(hs_cfg.flags & HS_CFG_FLAG_SET)) {
        ...
        /* Copy to user */
        if (ewl_os_memcpy(req->ifr_data, &hs_cfg, sizeof(ds_hs_cfg))) {
 ...
 */
void
woal_uap_set_multicast_list(moal_private *priv)
{
    ENTER();
 ...
 */
int
woal_uap_do_ioctl(moal_private *priv, int cmd, struct ifreq *req)
{
    int ret = 0;
    ENTER();
    ...
    switch (cmd) {
    case UAP_HOSTCMD:
        ret = woal_hostcmd_ioctl(priv, req);
        break;
    case UAP_IOCTL_CMD:
        ret = woal_uap_ioctl(priv, req);
        break;
    case UAP_POWER_MODE:
        ret = woal_uap_power_mode_ioctl(priv, req);
        break;
    case UAP_BSS_CTRL:
        ret = woal_uap_bss_ctrl_ioctl(priv, req);
        break;
    case UAP_WAPI_MSG:
        ret = woal_uap_set_wapi(priv, req);
        break;
    case UAP_BSS_CONFIG:
        ret = woal_uap_bss_cfg_ioctl(priv, req);
        break;
    case UAP_STA_DEAUTH:
        ret = woal_uap_sta_deauth_ioctl(priv, req);
        break;
    case UAP_GET_STA_LIST:
        ret = woal_uap_get_sta_list_ioctl(priv, req);
        break;
    case UAP_CUSTOM_IE:
        ret = woal_custom_ie_ioctl(priv, req);
        break;
    case UAP_GET_BSS_TYPE:
        ret = woal_get_bss_type(priv, req);
        break;
    case UAP_HS_CFG:
        ret = woal_uap_hs_cfg(priv, req);
        break;
    default:
        ret = woal_uap_do_priv_ioctl(priv, req, cmd);
        break;
    }
...
woal_uap_get_version(moal_private * priv, char *version, int max_len)
{
    ...
    if (status == MLAN_STATUS_SUCCESS) {
        ...
        ewl_os_snprintf(version, max_len, driver_version,
                 info->param.ver_ext.version_str);
    }
                   ...
                   mlan_ds_uap_stats * ustats)
{
    ...
    if (status == MLAN_STATUS_SUCCESS) {
        if (ustats)
            ewl_os_memcpy(ustats, &info->param.ustats, sizeof(mlan_ds_uap_stats));
    }
                        ...
                        mlan_uap_bss_param * sys_cfg)
{
    ...
    if (action == MLAN_ACT_SET) {
        ewl_os_memcpy(&bss->param.bss_config, sys_cfg, sizeof(mlan_uap_bss_param));
    }
    ...
    if (action == MLAN_ACT_GET) {
        ewl_os_memcpy(sys_cfg, &bss->param.bss_config, sizeof(mlan_uap_bss_param));
    }
...
woal_set_sys_config_invalid_data(mlan_uap_bss_param * config)
{
    ...
    ewl_os_memset(config, 0, sizeof(mlan_uap_bss_param));
