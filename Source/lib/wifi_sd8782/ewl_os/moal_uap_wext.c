...
woal_uap_get_freq(moal_private *priv, struct iw_freq *fwrq)
{
    ...
    fwrq->m = (long) (channel_to_frequency(ap_cfg.channel, band)) * 1000;
    fwrq->e = 1;
 ...
 */
static int
woal_config_commit(moal_private * priv,
                   struct iw_request_info *info, char *cwrq, char *extra)
{
    ENTER();
    MLAN_UNUSED_ARG(priv);
    MLAN_UNUSED_ARG(info);
    MLAN_UNUSED_ARG(cwrq);
 ...
 */
static int
woal_get_name(moal_private * priv, struct iw_request_info *info,
              char *cwrq, char *extra)
{
    ENTER();
    ewl_os_strcpy(cwrq, "IEEE 802.11-DS");
    MLAN_UNUSED_ARG(priv);
    MLAN_UNUSED_ARG(info);
    MLAN_UNUSED_ARG(extra);
    LEAVE();
             ...
             struct sockaddr *awrq, char *extra)
{
    ...
    if (priv->bss_started) {
        ewl_os_memcpy(awrq->sa_data, priv->current_addr, MLAN_MAC_ADDR_LENGTH);
    } else {
              ...
              struct iw_freq *fwrq, char *extra)
{
    ...
    ap_cfg = ewl_os_malloc(sizeof(mlan_uap_bss_param));
    if (ap_cfg == NULL) {
        ret = -ENOMEM;
        goto done;
    }

    sys_cfg = ewl_os_malloc(sizeof(mlan_uap_bss_param));
    if (sys_cfg == NULL) {
        ret = -ENOMEM;
        goto done;
    }
 ...
 */
static int
woal_get_freq(moal_private * priv, struct iw_request_info *info,
              struct iw_freq *fwrq, char *extra)
{
    mlan_uap_bss_param ap_cfg;
    int ret = 0;
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    if (dwrq->length) {
        ...
        if (!(dwrq->flags & IW_ENCODE_NOKEY)) {
            ...
            ewl_os_memcpy(pkey->key, extra, dwrq->length);
            /* Set the length */
        ...
        if (dwrq->flags & IW_ENCODE_DISABLED) {
            ...
            if (key_index < 0) {
                ...
                if (key_index == 0)
                    ewl_os_memcpy(pkey, &ap_cfg->wep_cfg.key0, sizeof(wep_key));
                if (key_index == 1)
                    ewl_os_memcpy(pkey, &ap_cfg->wep_cfg.key1, sizeof(wep_key));
                if (key_index == 2)
                    ewl_os_memcpy(pkey, &ap_cfg->wep_cfg.key2, sizeof(wep_key));
                if (key_index == 3)
                    ewl_os_memcpy(pkey, &ap_cfg->wep_cfg.key3, sizeof(wep_key));
                /* Set current key index as default */
                pkey->is_default = MTRUE;
            }
        }
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    case PROTOCOL_STATIC_WEP:
        ...
        if (pkey) {
            ...
            ewl_os_memcpy(extra, pkey->key, pkey->length);
            dwrq->flags &= ~IW_ENCODE_DISABLED;
        } else {
            ret = -EFAULT;
        }
    ...
    case PROTOCOL_WPA2_MIXED:
        ewl_os_memcpy(extra, ap_cfg.wpa_cfg.passphrase, ap_cfg.wpa_cfg.length);
                ...
                struct iw_point *dwrq, char *extra)
{
    ...
    ewl_os_memcpy(misc->param.cust_ie.ie_data_list[0].ie_buffer, extra, dwrq->length);
                    ...
                    struct iw_point *dwrq, char *extra)
{
    ...
    if ((dwrq->flags & IW_ENCODE_DISABLED) && !ext->key_len) {
        ...
        ewl_os_memcpy(pwep_key->key, pkey_material, ext->key_len);
    } else {
        sys_cfg.protocol = PROTOCOL_WPA2_MIXED;
        ...
        ewl_os_memcpy(sec->param.encrypt_key.key_material, pkey_material,
               ext->key_len);
        ewl_os_memcpy(sec->param.encrypt_key.mac_addr, ext->addr.sa_data, ETH_ALEN);
              ...
              struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
    ...
    ewl_os_memset(sta_addr, 0, ETH_ALEN);
    if ((mlme->cmd == IW_MLME_DEAUTH) || (mlme->cmd == IW_MLME_DISASSOC)) {
        ewl_os_memcpy(sta_addr, (t_u8 *) mlme->addr.sa_data, ETH_ALEN);
        ...
        if (!ewl_os_memcmp(bc_addr, sta_addr, ETH_ALEN)) {
            ...
            sta_list =
                (mlan_ds_sta_list *) ewl_os_malloc(sizeof(mlan_ds_sta_list));
            ...
            ewl_os_memcpy(sta_list, &pinfo->param.sta_list, sizeof(mlan_ds_sta_list));
            if (req)
                ewl_os_free(req);
        }
        ...
        if (!ewl_os_memcmp(bc_addr, sta_addr, ETH_ALEN)) {
            for (i = 0; i < sta_list->sta_count; i++) {
                ewl_os_memcpy(bss->param.deauth_param.mac_addr,
            ...
            ewl_os_memcpy(bss->param.deauth_param.mac_addr, sta_addr, ETH_ALEN);
            bss->param.deauth_param.reason_code = mlme->reason_code;
 ...
 */
static int
woal_set_priv(moal_private * priv, struct iw_request_info *info,
              struct iw_point *dwrq, char *extra)
{
    ENTER();
    MLAN_UNUSED_ARG(info);
 ...
 */
static int
woal_set_essid(moal_private * priv, struct iw_request_info *info,
               struct iw_point *dwrq, char *extra)
{
    mlan_uap_bss_param sys_cfg;
    int ret = 0;
    ...
    ewl_os_memcpy(sys_cfg.ssid.ssid, extra,
               ...
               struct iw_point *dwrq, char *extra)
{
    ...
    if (priv->bss_started) {
        ...
        ewl_os_memcpy(extra, ap_cfg.ssid.ssid, dwrq->length);
    } else
        dwrq->length = 0;
