...
static SFLScontrol sfls_mx25l12835f = {
	...
	.mode_en = MX25L12835F_READ_MODE_EN,
	.mode_val = MX25L12835F_READ_MODE_VALUE,
...
#if (MX25L12835F_ENABLE_QPI==1)
void SflsMx25l12835f_EnterQPI(ENX_YN doReq)
{
	if (sfls_mx25l12835f.is_qpi == 0) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(MX25L12835F_CMD_ENTER_QPI, 0);
		} else {
...
void SflsMx25l12835f_ExitQPI(ENX_YN doReq)
{
	if (sfls_mx25l12835f.is_qpi == 1) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(MX25L12835F_CMD_EXIT_QPI, 2);
		} else {
