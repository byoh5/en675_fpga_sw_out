...
static SFLScontrol sfls_is25lp256 = {
	...
	.mode_en = IS25LP256_READ_MODE_EN,
	.mode_val = IS25LP256_READ_MODE_VALUE,
...
inline UINT SflsIs25lp256_Is4B(void)
{
...
void SflsIs25lp256_Enter4B(ENX_YN doReq)
{
	if (SflsIs25lp256_Is4B() == ENX_NO) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(IS25LP256_CMD_EN4B, 0);
		} else {
			SflsSingleCommand(IS25LP256_CMD_EN4B, 0);
		}
	} else {
...
void SflsIs25lp256_Exit4B(ENX_YN doReq)
{
	if (SflsIs25lp256_Is4B() == ENX_YES) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(IS25LP256_CMD_EX4B, 2);
		} else {
			SflsSingleCommand(IS25LP256_CMD_EX4B, 2);
		}
	} else {
...
#if (IS25LP256_ENABLE_QPI==1)
inline UINT SflsIs25lp256_IsQPI(void)
{
	return sfls_is25lp256.is_qpi;
}

void SflsIs25lp256_EnterQPI(ENX_YN doReq)
{
	if (SflsIs25lp256_IsQPI() == ENX_NO) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(IS25LP256_CMD_ENTER_QPI, 0);
		} else {
			SflsSingleCommand(IS25LP256_CMD_ENTER_QPI, 0);
		}
		sfls_is25lp256.is_qpi = ENX_YES;
	} else {
		ENX_DEBUGF(DBG_SFLS_WARNING, "already enter QPI\n");
	}
}

void SflsIs25lp256_ExitQPI(ENX_YN doReq)
{
	if (SflsIs25lp256_IsQPI() == ENX_YES) {
		if (doReq == ENX_NO) {
			SflsSingleCommandNoReq(IS25LP256_CMD_EXIT_QPI, 2);
		} else {
			SflsSingleCommand(IS25LP256_CMD_EXIT_QPI, 2);
		}
		sfls_is25lp256.is_qpi = ENX_NO;
	} else {
