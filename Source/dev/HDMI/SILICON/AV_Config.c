...
BYTE InitVideo(byte TclkSel)
{
	...
	_printf(" V : %d\r\n", (int) VideoModeDescription.VIC, 8);
	_printf(" A : %d\r\n", (int) VideoModeDescription.AspectRatio, 8);
	_printf(" CS: %d\r\n", (int) VideoModeDescription.ColorSpace, 8);
	_printf(" CD: %d\r\n", (int) VideoModeDescription.ColorDepth, 8);
	_printf(" HA: %d\r\n", (int) VideoModeDescription.HDCPAuthenticated, 8);
	_printf(" HF: %d\r\n", (int) VideoModeDescription.HDMIVideoFormat, 8);
	_printf(" 3D: %d\r\n", (int) VideoModeDescription.ThreeDStructure, 8);
...
#endif
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	B_Data[1] = (BITS_OUT_RGB | BITS_OUT_AUTO_RANGE) & ~BIT_BT_709;										// 0x10
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
byte SetDE(void)
{
    ...
    if (VModesTable[ModeTblIndex]._3D_Struct != NO_3D_SUPPORT)
    {
        ...
        _printf((">>SetDE() not allowed with 3D video format\n"),0,0);
    }
...
byte ChangeVideoMode(void)
{
  	...
  	_printf((">>converts to tabel indexChangeVideoMode()\n"),0,0);
    ...
    else                                                // Command length == 7. AVI InfoFrame is set by the FW
    {
        if ((Arg(3) & TWO_LSBITS) == 1)         // AVI InfoFrame DByte1
            ...
            if (((Arg(6) >> 2) & TWO_LSBITS) == 3)                  // Extended colorimetry - xvYCC
            {
                ...
                B_Data[2] = BT_709;                                           // AVI Byte2: C1C0

            else if (((Arg(6) >> 2) & TWO_LSBITS) == 1)             // BT.601
                B_Data[2] = BT_601;                                           // AVI Byte2: C1C0
