...
#define BT656_DDRMODE_TEST


#ifdef BT656_DDRMODE_TEST
static int video_format = TP2802_720P30;
...
void TVI_Init(void)
{
	...
	tp28xx_byte_write(chip, 0x02, 0xC2 | BT656_HEADER_8BIT); //BT656 8bit mode 720 decoding
	...
	tp2827c_set_video_mode(chip, video_format, DEFAULT_CHANNEL, STD_HDA); // AHD mode Initial
	printf("TP2827 - // AHD mode Initial..\r\n");

	tp28xx_byte_write(chip, 0xFA, 0x99); // 148.5 -> 74.25
	//tp28xx_byte_write(chip, 0xFA, 0x11); // 148.5 -> 74.25
	...
	//tp28xx_byte_write(chip, 0x40, 0x00);
	_Yprintf("0x02 : 0x%02x\r\n", tp28xx_byte_read(chip, 0x02));
	_Yprintf("0x03 : 0x%02x\r\n", tp28xx_byte_read(chip, 0x03));
	_Yprintf("0x2C : 0x%02x\r\n", tp28xx_byte_read(chip, 0x2C));
	_Yprintf("0x40 : 0x%02x\r\n", tp28xx_byte_read(chip, 0x40));
